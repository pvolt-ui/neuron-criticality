...
> ## Revision note (2026-08-19): the "visual" pathway was the ocellar pathway, and Null B now has a selection-bias control
>
> Two corrections were made after review. Both change numbers below; everything
> affected is marked **[legacy hybrid]** and the corrected figures are given
> alongside.
>
> **1. The pathway previously called "visual" is renamed `ocellar`, and a
> spec-compliant `visual` pathway has been built.** The old sources were
> `class == visual` (photoreceptors). Of the 146 that survived into the subgraph,
> **142 were ocellar photoreceptors and only 4 were compound-eye
> photoreceptors** — and almost the entire 24,185-node subgraph was spawned by
> those 4 (the pure ocellar pathway is **386 nodes / 2,095 edges**). So the old
> "visual" ranking was an ocellar reflex pathway with a large medulla halo
> hanging off four neurons, which is why its raw top-50 was medulla-heavy and
> why its Null B z-scores were the highest of the three: in a 24k-node graph,
> being on the ocellar exit route is trivially non-random. Re-running on the
> pure ocellar graph, **Null B survival drops from 45/50 to 13/50** (top-25:
> 24/25 → 12/25, median z 19.9 → 0.55) and `OCG01d/e/b` sit at z ≈ 0 — their
> rank is explained by their degree. The ocellar pathway is real, small, and
> *not* a positional-bottleneck story.
>
> The new `visual` pathway uses the lamina/medulla columnar input neurons the
> spec asked for (curated families Lamina Monopolar + Transmedullary + Medulla
> Intrinsic; 17,004 sources, 60,467 nodes, 1,231,957 edges, 422 descending
> targets). Exact all-source betweenness is infeasible there, so it uses the
> study's sampled-source protocol (500 sources, seed 0). Its top-25 is clean
> motion-vision anatomy: the wide-field GABAergic lobula-plate/lobula
> interneurons (`LPi15`, `LPi14`, `Am1`, `CT1`, `Li31`, `Sm41/42`), the
> horizontal-system cell `H2`, lobula tangentials (`LT62`, `LT79`, `LT1d`,
> `LPT26`), then the AVLP/PVLP/IPS relays (`PVLP015`, `PVLP011`, `AVLP435a`,
> `PS124`, `WED075`). Source definitions now live in `pathways.py`; every node
> file carries an `is_source` column.
>
> Also: MCNS annotates **no ocellar photoreceptors** — its `class == visual`
> neurons are compound-eye R1–R8. The MCNS pathway formerly compared against
> FAFB-"visual" is renamed `photoreceptor` and the earlier "visual replicates
> in MCNS (10/25, ρ = 0.45)" claim is **withdrawn**: it compared an ocellar
> source set against an R7/R8 source set. The new lamina/medulla `visual`
> pathway is rebuilt in MCNS (29,459 sources, 83,016 nodes) and compared
> like-for-like; see the replication section.
>
> **2. Null B had an uncontrolled winner's-curse.** The top-50 are selected by
> *real* betweenness and then tested against their own per-node null — but
> nodes selected for being extreme will sit above their null mean even in a
> graph with no positional structure. `nullB_selection_control.py` runs the
> entire Null B procedure on a degree-preserving shuffle treated as data; every
> "survivor" there is a false positive by construction. See the new section
> "Null B selection-bias control". Short version: the floor is ~3/50 in ocellar
> and **~15/50 in olfactory**, so olfactory's 37/50 is roughly 22/50 net of
> selection, and survivors with z < ~7 cannot be distinguished from selection
> alone.
>
> Legacy-graph numbers are retained where the analysis has not been re-run
> (influence, metric matrix, deletion audit, Null A across all three, MCNS
> degree-independence for the photoreceptor pathway) and are flagged.
...
## What's done: Phase A + B (ranked bottlenecks, four pathways) + Phase D (cross-modal)

For each sensory pathway (olfactory, mechanosensory, visual, ocellar — see revision note):
...
| **Visual** (lamina/medulla inputs: L1–L5, Tm, Mi families) | 17,004 | 422 | 60,467 | 1,231,957 |
| **Ocellar** (ocellar photoreceptors) | 142 | 90 | 386 | 2,095 |
| ~~Visual (photoreceptors)~~ **[legacy hybrid]** — 142 ocellar + 4 compound-eye PRs | 146 | 105 | 24,185 | 236,798 |

Source definitions: `pathways.SOURCE_DEFINITION`. Visual betweenness is computed
under the sampled-source protocol (500 of 17,004 sources, seed 0;
`ranked_visual.PROTOCOL.txt`) because exact all-source betweenness on a
60k-node / 1.2M-edge graph is infeasible; olfactory, mechanosensory and ocellar
are exact.
...
**Visual — lamina/medulla inputs (new; sampled-source protocol)** — the top-50 is
20 optic-lobe intrinsic, 9 visual-projection and 21 central neurons; 28 GABAergic,
20 cholinergic. The top of the list is the wide-field inhibitory network of the
lobula plate and lobula — `LPi15` ×2, `Am1` ×2, `LPi14` ×2, `CT1` ×2, `Sm41`, `Sm42`,
`Li31` ×2 — followed by the lobula/lobula-plate tangential outputs (`LT62`, `LT79`,
`H2`, `LPT26`, `LT1d`) and then the central relays the other pathways also surface
(`PVLP015`, `PVLP011`, `AVLP435a`, `PS124`, `WED075`, `PLP018`). Neuropils: `LO_R` 8,
`AVLP_R` 7, `LOP_R` 6, `LO_L` 6. Read: every route from a columnar visual input to a
descending neuron funnels through the lobula-plate wide-field inhibitory layer and
the tangential output cells — the textbook motion-vision exit. Null B for this
pathway: see the Null B section (30 trials; 29/50, top-25 16/25, median z 3.0 — the optic-lobe wide-field cells `LPi15`, `Am1`, `CT1`, `Li31`, `PVLP011` sit at z ≈ 0 and are degree-explained; the survivors are the central relays `PS124` z = 15.1, `AVLP435a` z = 8.3, `PVLP015` z = 4.4, `Sm41` z = 5.3, `LT79` z = 3.8).

**Ocellar (12/25 validated, median z = 0.55; 200 trials, pure 386-node graph)** — the
honest version of the old "visual" story. The raw top-25 is `OCG01d/e/b/f` ×9,
`OCG02b` ×2, `OCC01b`, descending `DNp28` ×2, `DNp19` ×2, `DNp40`, `DNb06`, and central
`CB0567`, `CB1350`, `CB0415`, `CB0392`, `PS126`, `CB0676`, `PS276`, `CB2126`; 25 ACH,
18 GLUT, 6 GABA; `IPS_L/R` dominate. But on the pure ocellar graph the OCG cells
themselves **do not beat the degree-preserving null** (`OCG01d` z = 0.9 / −0.3,
`OCG01e` z = 0.1, `OCG01b` z = 0.0): the ocellar graph is so small (112 neurons with
nonzero betweenness) that OCG rank is fully explained by connection count. The
survivors are the descending neurons `DNp19` (z = 13.3), `DNp28` (z = 6.8) and a few
central relays. Null A (weight permutation) also fails here (z = −0.43, p = 0.78) and
weighting changes only 12/50 of the top-50. **Ocellar is a degree-driven reflex arc,
not a positional-bottleneck pathway.**

~~**Visual (24/25 validated, median z = 35.1)**~~ **[legacy hybrid — superseded by
the two paragraphs above; kept for the record]** — **validation moves the story out of the
...
**Revised (four pathways: olfactory, mechanosensory, visual, ocellar).** Of the 200
top-50 candidates, **six neurons appear in two pathways' top-50. None appears in
three or more.** (`cross_modal_overlap.py`, re-run 2026-08-19.)

| root_id | cell type | neuropil | NT | pathways |
|---|---|---|---|---|
| 720575940615723314 | PVLP076 | AVLP_R | ACH | mechanosensory + olfactory |
| 720575940608545219 | AVLP080 | AVLP_R | GABA | mechanosensory + visual |
| 720575940627796298 | AVLP080 | AVLP_L | GABA | mechanosensory + visual |
| 720575940623702345 | CB0677 | VES_R | GABA | mechanosensory + visual |
| 720575940619636615 | PS124 | IPS_R | ACH | mechanosensory + visual |
| 720575940643112333 | CB0676 | SPS_R | ACH | mechanosensory + ocellar |

`PS124` and `CB0676` carry over from the legacy list; `AVLP080` (both hemispheres) and
`CB0677` are new, and come from the real visual pathway. The legacy list
**[legacy hybrid]** was:

| root_id | cell type | neuropil | NT | modalities (legacy) |
|---|---|---|---|---|
| 720575940615723314 | PVLP076 | AVLP_R | ACH | mechanosensory + olfactory |
...
The deletion-test and cohort sections below were run on the legacy five; they have not
been re-run on `AVLP080` / `CB0677`.
...
loud, because "betweenness centrality was used here" implies the ranking contains something
...
| **Ocellar** (pure 386-node graph, 200 trials) | **13/50** | **12/25** | **0.55** |
| **Visual** (lamina/medulla, 30 trials) | 29/50 | 16/25 | 3.0 |
| ~~Visual~~ **[legacy hybrid]** | 45/50 | 24/25 | 19.9 |

**The ocellar row is the correction that matters.** The legacy "visual" 45/50 was
computed on the 24k-node hybrid graph; on the pure ocellar graph the OCG cells fall to
z ≈ 0 and only the descending neurons (`DNp19` z = 13.3, `DNp28` z = 6.8) and a few
central relays survive. The sentence "the z-scores are the highest of the three
because in a graph that redundant, being on the strong route at all is highly
non-random" was describing the 4-photoreceptor halo, not the ocellar pathway, and is
withdrawn.
...
### Null B selection-bias control — how much of "survival" is winner's curse?

Null B selects the top-50 by *real* betweenness and then tests each against its own
null distribution. Nodes selected for being extreme will sit above their null mean
even when there is no positional structure at all, so the survival counts above have
an unknown false-positive floor. `nullB_selection_control.py` measures it: one
degree-preserving shuffle is treated as the data, *its* top-50 are taken, and those
are tested against further shuffles with the identical procedure (same seed-0 source
sample, same swap factor, same BH-FDR on p_z). Every survivor there is a false
positive by construction.

| Modality | real survivors (top-50 / top-25) | control survivors (mean ± sd) | real median z / max z | control median z / max z |
|---|---|---|---|---|
| Olfactory (3 pseudo-real × 50 trials) | 37/50 · 20/25 | **16.0 ± 0.8** · 7.3 ± 2.5 | 5.3 / 35.5 | 1.18 / 8.1 |
| Mechanosensory (3 × 50) | 43/50 · 20/25 | **34.0 ± 2.2** · 15.3 ± 2.4 | 6.4 / 31.1 | 2.73 / 14.6 |
| Ocellar (5 × 200) | 13/50 · 12/25 | 2.8 ± 3.4 · 1.4 ± 1.5 | 0.55 / 237 | 0.55 / 3.4 |
| Visual | not run (one evaluation ≈ 4 min; 3 × 30 trials ≈ 6 h) | | | |

Read:

- **Selection alone manufactures survivors, and the number is large.** A structureless
  olfactory graph yields 16/50 "degree-independent bottlenecks" (median z 1.2, max z 8);
  a structureless **mechanosensory** graph yields **34/50** (median z 2.7, max z up to
  23). So olfactory's 37/50 is roughly 21/50 net of selection, and mechanosensory's
  43/50 — previously "the strongest pathway on every check" — is only ~9/50 above its
  own floor. In mechanosensory, survivors with z below ~15 cannot be distinguished from
  the winner's curse; in olfactory the bar is z ≈ 8. The named survivors with z = +12 to +50
  (`MZ_lv2PN`, `v2LN30`, the `lLN2T_c` copies) are well clear of that; the marginal
  ones — including the one surviving `lLN2F_b` copy at z = +2.1 — are not, and
  should be struck rather than "qualified".
- **Ocellar's 13/50 is barely above its own floor (2.8 ± 3.4)** and its median z
  (0.55) *equals* the control's. The ocellar pathway has essentially no positional
  signal beyond degree.
- **Visual (29/50, median z 3.0, 30 trials)** was not controlled, but on the
  mechanosensory precedent a floor of 20–30/50 is plausible; only `PS124` (z = 15),
  `AVLP435a` (z = 8.3) and `Sm41` (z = 5.3) are likely real survivors.
- The control calibrates the z-scale per pathway: the max z seen in any pseudo-real
  graph was 8 (olfactory), 23 (mechanosensory), 3.4 (ocellar). That per-pathway max —
  not q ≤ 0.05 — is the practical bar for calling a single neuron degree-independent.
  **The headline "Null B rescues the ranking" should be downgraded to: above the
  strictest bar — the largest z any pseudo-real graph produced (olfactory 9.9,
  mechanosensory 23.4) — 15/50 olfactory neurons (`MZ_lv2PN` z = 35/27, `VES079` 23,
  `AL-AST1` 21/20, `lLN2T_c` 17/12, `VESa1_P02` 16/12, `v2LN30` 13, `LHPV10b1` 13/10,
  `CB0448`, `DNpe002`, `LHPD5c1`) and only **2/50 mechanosensory** neurons (`DNge132`
  z = 31, `CB0021` z = 23) are degree-independent beyond reasonable doubt. Against a
  looser bar (z > 15, the control's typical max) mechanosensory has 8 (`DNg62` 23/16,
  `DNg85` 23, `DNg98` 18, `AN_GNG_89` 18). The remaining FDR survivors are not
  distinguishable from selection.**

**This rescues the ranking from the degree critique — for the strong survivors.** The two findings are not in conflict:
...
| **Visual** (lamina/medulla: L1–5, Tm, TmY, Mi types) | 60,467 / 1,231,957 | 83,016 / 2,908,395 | 29,459 | 743 |
| Ocellar | 386 / 2,095 | **not buildable** — MCNS annotates no ocellar photoreceptors | — | — |
| MCNS `photoreceptor` (class == visual = compound-eye R1–R8) **[legacy, was "visual"]** | (no FAFB counterpart) | 45,794 / 977,533 | 254 | 71 |

MCNS is a substantially denser reconstruction (6.24M weighted pairs against FAFB's 2.70M).
**Correction:** the earlier sentence here — "the visual pathway thins to 254 sources,
mirroring FAFB's 146 — the same structural fact in a second animal" — was wrong. The
FAFB 146 were ocellar photoreceptors; the MCNS 254 are R7/R8. Two different source
populations happened to both be small. MCNS's visual-class pathway is kept under the
name `photoreceptor` and is not a replication of anything in FAFB.
...
| **Visual** (lamina/medulla, like-for-like, 150 sampled sources each) | 2,323 | **13** | 0.27 | 2.9e-21 | **+0.704** |
| ~~Visual~~ **[withdrawn]** — compared FAFB ocellar PRs vs MCNS R7/R8 | 282 | 10 | 2.22 | 6.5e-06 | +0.449 |

**A caveat on ρ that applies to every row.** Spearman over *all* shared types includes
the large majority with betweenness exactly 0 in both datasets, so part of ρ is "the
same types are in the pathway at all", not "the same types rank high". The top-25
overlap against its hypergeometric expectation is the number to lead with. `replication_specificity.py` runs
the sharper test — ρ restricted to types with nonzero betweenness in both, and the
identical test on a `total_syn` (in + out synapses within the pathway subgraph, max per
type) ranking:

| Pathway | betweenness: top-25 overlap / ρ all / ρ nonzero-in-both (n) | **total_syn baseline**: top-25 overlap / ρ |
|---|---|---|
| Olfactory | 9 / +0.67 / +0.54 (157) | **20** / **+0.90** |
| Mechanosensory | 12 / +0.71 / +0.56 (639) | **18** / **+0.86** |
| Visual | 13 / +0.70 / +0.60 (548) | **19** / **+0.91** |

**Degree replicates across animals better than betweenness does**, in every pathway.
So "the bottleneck rankings agree at ρ ≈ 0.7 across thousands of cell types" is true but
is not evidence that *betweenness* carries conserved information — it is evidence that
the pathway's cell-type composition and synapse budget are conserved, which any
degree-like score inherits. The defensible statement is narrower: the betweenness
ranking is conserved *about as well as one would expect given that degree is conserved*,
with ρ ≈ 0.55–0.60 among the types that actually carry paths. The sentence "this is the
strongest result in the study" below should be read with that qualifier.
...
- **Visual** (lamina/medulla, new) — `Am1`, `H2`, `LPT26`, `LT11`, `LT1c`, `LT62`,
  `LT79`, `Li32`, `Li33`, `PVLP011`, `PVLP061`, `AVLP080`, `PS124`. The lobula-plate
  wide-field / tangential-output story reproduces in the male CNS at the highest overlap
  of the three pathways (13/25 against 0.27 expected). `AVLP080` and `PS124` replicate in
  both mechanosensory and visual — the two cross-modal neurons from the revised Phase D
  list.
- ~~Visual~~ **[withdrawn]** — the previously listed `aMe12`, `aMe6a`, `aMe20`, `Li33`,
  `PS279`, `PS280`, `DNp12`, `DNb05`, `OA-AL2i1`, `PS124` overlap was between the FAFB
  ocellar/4-PR hybrid and MCNS R7/R8 — not the same pathway in two animals. The
  sentence "two independent lines now say the same thing" is retracted.
...
### Degree-independence replicates (olfactory; the "visual" row is legacy)
...
| ~~Visual~~ **[legacy: FAFB hybrid vs MCNS photoreceptor — different pathways]** | 45/50 | 46/50 | 24/25 | 24/25 | 19.9 | 12.1 |

**Olfactory replicates within one or two neurons at every threshold.** (The visual row
is kept for the record only; on the pure FAFB ocellar graph the count is 13/50, and
MCNS has no ocellar pathway to compare it with.) `v2LN30` tops both
...
Both halves of the mechanosensory result are corroborated: the pathway really is
...
- **The ocellar pathway is tiny and degree-driven.** 142 sources, 90 targets, 386 nodes,
  112 neurons with nonzero betweenness; Null B 13/50 against a selection floor of ~3/50;
  Null A fails. It is reported for completeness, not as a bottleneck result.
- **The visual pathway uses sampled sources (500 of 17,004) and a 30-trial Null B**, so
  its ranking has sampling noise the other two do not, and its Null B p-floor is 1/31.
  Its selection-bias control was not run (≈6 h).
- **Legacy "visual" numbers remain in the influence / metric-matrix / deletion-audit /
  Null A / cohort sections** and are flagged **[legacy hybrid]**. Ocellar has been re-run
  for Null A, weighting ablation, cell-type ablation and cohort sweep; visual has not.
...
- **"Bottleneck" means high rank, not indispensable.** The deletion test puts a hard
  ...
  pair.
- **Two datasets now (FAFB v783 + MCNS v0.9).** Replicated in MCNS: the *ranking*
  (olfactory, mechanosensory, and — new — lamina/medulla visual, cell-type level), the
  *redundancy* result (olfactory, mechanosensory; the "visual" row is the legacy
  photoreceptor pathway), and *Null B* (olfactory only — the old visual row compared
  different pathways). Still FAFB-only: Null B for mechanosensory (~6-8 h, not
...
- **Null B uses sampled sources** (100 per modality; 200 trials olfactory /
  mechanosensory / ocellar, 30 visual) so that the shuffled-graph betweenness runs stay
  affordable. The sample is identical across real and null graphs so
...
0. **Re-run the legacy-flagged analyses on the real pathways**: deletion audit on the
   new cross-modal six; influence + metric matrix on visual and ocellar; cohort sweep and
   cell-type ablation on visual; Null A on visual; the Null B selection control on
   visual. Regenerate `fig1`–`fig5` with four pathways.
...
python3 build_pathway_subgraphs.py    # weighted subgraphs: olfactory, mechanosensory, visual, ocellar
python3 betweenness.py --modality olfactory mechanosensory ocellar   # exact  (~25 min)
python3 betweenness.py --modality visual --sample 500                # sampled-source (~20 min)
...
python3 null_degree_preserving.py --trials 200   # Null B (olf/mech/ocellar); --modality visual --trials 30 (~2 h)
python3 nullB_selection_control.py  # Null B winner's-curse floor (olf ~35 min, mech ~80 min, ocellar 1 min)
...
python3 prepare_mcns.py [--endpoint motor]     # second connectome; builds olfactory, mechanosensory, visual, photoreceptor
